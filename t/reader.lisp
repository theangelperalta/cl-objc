...
(test typed-more-params
  ...
  (is (string-equal (eval 
		     (read-from-string "[@\"abc\" stringByPaddingToLength: :int 9 withString: objc-id @\".\" startingAtIndex: :int 0]")) 
		    "abc......"))
  (restore-readtable))
...
(test untyped-basic-reading
  (activate-objc-reader-macro nil)
  (is (equal (read-from-string "[NSObject alloc]") 
	     '(untyped-objc-msg-send ((objc-get-class "NSObject") "alloc"))))
  (restore-readtable))
...
(test untyped-more-params
  (activate-objc-reader-macro nil)
  (is (string-equal (eval 
		     (read-from-string "[@\"abc\" stringByPaddingToLength: 9 withString: @\".\" startingAtIndex: 0]")) 
		    "abc......"))
  (restore-readtable))
