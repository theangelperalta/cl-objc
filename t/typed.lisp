...
a light struct as input parameter"
      (let ((range (cffi:foreign-alloc 'struct-ns-range)#+(or)(cl-objc::make-ns-range :location 100 :length 99))
	...
	(setf (cffi:foreign-slot-value range 'struct-ns-range 'location) intval1
        ...
        (let ((value-with-range (typed-objc-msg-send ((objc-get-class "NSValue") "valueWithRange:") (:struct struct-ns-range) (cffi:convert-from-foreign range '(:struct struct-ns-range)))))
...
big struct as input parameter"
	...
	(let ((floatval (coerce (random 4.0) 'double-float)))
	  ...
	  (let ((value-with-rect (typed-objc-msg-send ((objc-get-class "NSValue") "valueWithRect:") (:struct struct-cg-rect) rect)))
	    (is (= floatval (cl-objc::cg-size-width (cl-objc::cg-rect-size (cffi:convert-from-foreign (typed-objc-msg-send (value-with-rect "rectValue")) '(:struct struct-cg-rect))))))))))
